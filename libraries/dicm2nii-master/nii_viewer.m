...
% Global variable for spectrum window management
% persistent MRSI_SPECTRUM_WINDOW;
...
if nargin>1 && any(ishandle(varargin{1})) % called by Open or dnd
    ...
    if isempty(pf) || ~isfield(pf, 'layout') % check lastly-added field
        pf = struct('openPath', pwd, 'addPath', pwd, 'interp', 'linear', ...
            ...
            'mouseOver', true, 'layout', 2, 'autoSpectrum', true);
        setpref('nii_viewer_para', fieldnames(pf), struct2cell(pf));
    end
    
    % Add autoSpectrum field if missing (for backward compatibility)
    if ~isfield(pf, 'autoSpectrum')
        pf.autoSpectrum = true;
        setpref('nii_viewer_para', 'autoSpectrum', pf.autoSpectrum);
    end
...
uimenu(h_view, 'Label', 'Show colorbar', 'Callback', cb('colorbar'));
uimenu(h_view, 'Label', 'Clear spectrum', 'Callback', cb('clearSpectrum'));
...
switch cmd
    ...
    case 'clearSpectrum'
        if ishandle(hs.ax(4))
            cla(hs.ax(4));
            set(hs.ax(4), 'Visible', 'off');
        end
        % Restore colorbar if it was visible before
        if ishandle(hs.colorbar) && isvalid(hs.colorbar)
            set(hs.colorbar, 'Visible', 'on');
        end
    case 'mousedown' % image clicked - enhanced for MRSI
        ax = gca;
        ...
        % Check if clicked data is MRSI and show spectrum
        try
            pf = getpref('nii_viewer_para');
            autoSpectrum = true; % Default to true
            if isfield(pf, 'autoSpectrum')
                autoSpectrum = pf.autoSpectrum;
            end
            
            if autoSpectrum
                jf = hs.files.getSelectedIndex+1;
                fprintf('Selected file index: %d\n', jf);
                if jf >= 1
                    p = get_para(hs, jf);
                    fprintf('File name: %s\n', p.nii.hdr.file_name);
                    ...
                    % Debug: Check if MRSI detection is working
                    fprintf('Checking MRSI data...\n');
                    fprintf('Volume count: %d\n', size(p.nii.img, 4));
                    fprintf('Intent code: %d\n', p.nii.hdr.intent_code);
                    fprintf('Is MRSI: %d\n', is_mrsi_data(p));
                    
                    if is_mrsi_data(p)
                        fprintf('MRSI detected, showing spectrum...\n');
                        show_mrsi_spectrum_onclick(hs, p, c, ax);
                    else
                        fprintf('MRSI not detected\n');
                    end
                end
            end
        catch ME
            fprintf('Error in MRSI detection: %s\n', ME.message);
        end
...
if ischar(fname)
    ...
    % === LOAD COMPANION ftSpec_smooth_w .mat FILE ===
    [p_dir, p_name, p_ext] = fileparts(fname);
    if endsWith(p_name, '.nii')
        p_name = extractBefore(p_name, '.nii');
    end
    ftSpecPath = fullfile(p_dir, [p_name '_ftSpec.mat']);
    
    if exist(ftSpecPath, 'file')
        fprintf('Loading companion ftSpec_smooth_w from: %s\n', ftSpecPath);
        try
            ftSpec_data = load(ftSpecPath);
            if isfield(ftSpec_data, 'ftSpec_smooth_w')
                p.ftSpec_smooth_w = ftSpec_data.ftSpec_smooth_w;
                fprintf('Successfully loaded ftSpec_smooth_w for FID-A processing\n');
            else
                fprintf('Warning: ftSpec.mat found but does not contain ftSpec_smooth_w\n');
                p.ftSpec_smooth_w = [];
            end
        catch ME
            fprintf('Warning: Could not load ftSpec.mat: %s\n', ME.message);
            p.ftSpec_smooth_w = [];
        end
    else
        p.ftSpec_smooth_w = [];
    ...
    p.ftSpec_smooth_w = []; % No ftSpec when nii struct is passed directly
end
...
hs.autoSpectrum = uicontrol(d, 'Style', 'checkbox', ...
    'Position', [8 150 380 22], 'Value', pf.autoSpectrum, ...
    'String', 'Automatically show spectrum when clicking MRSI voxels', ...
    'TooltipString', 'Enable automatic spectrum display for MRSI data');
...
if strcmp(get(h, 'Tag'), 'OK') % done
    ...
    pf.autoSpectrum = get(hs.autoSpectrum, 'Value');
    setpref('nii_viewer_para', fieldnames(pf), struct2cell(pf));
    delete(get(h, 'Parent'));
...
if ~ishandle(hs.colorbar) || ~isvalid(hs.colorbar), return; end
...
% Load companion ftSpec_smooth_w .mat file if it exists
% [p_dir, p_name, p_ext] = fileparts(fname);
% if endsWith(p_name, '.nii')
%     p_name = extractBefore(p_name, '.nii');
% end
...
%         ftSpec_data = load(ftSpecPath);
%         if isfield(ftSpec_data, 'ftSpec_smooth_w')
%             ftSpec_smooth_w_global = ftSpec_data.ftSpec_smooth_w;
%             fprintf('Successfully loaded ftSpec_smooth_w for FID-A processing\n');
%         else
%             fprintf('Warning: ftSpec.mat found but does not contain ftSpec_smooth_w\n');
%             ftSpec_smooth_w_global = [];
%         end
%     catch ME
%         fprintf('Warning: Could not load ftSpec.mat: %s\n', ME.message);
%         ftSpec_smooth_w_global = [];
%     end
% else
%     fprintf('No companion ftSpec.mat found (optional)\n');
%     ftSpec_smooth_w_global = [];
% end
hdr = p.nii.hdr;
